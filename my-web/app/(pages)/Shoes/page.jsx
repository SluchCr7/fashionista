...
export default function ShoesPage() {
    ...
    const [filterState, setFilterState] = useState({
        query: '', sort: 'featured', gender: 'All', priceRange: [0, 2500], brands: []
    });
    const [mobileFiltersOpen, setMobileFiltersOpen] = useState(false);
    const [page, setPage] = useState(1);
    const perPage = 12;

    const brandsList = useMemo(() => Array.from(new Set(products.map(p => p.brand).filter(Boolean))), [products]);
    ...
    const toggleBrand = (brand) => {
        setFilterState(prev => ({ ...prev, brands: prev.brands.includes(brand) ? prev.brands.filter(b => b !== brand) : [...prev.brands, brand] }));
        setPage(1);
    };
    const resetFilters = () => setFilterState({ query: '', sort: 'featured', gender: 'All', priceRange: [0, 2500], brands: [] });
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-[#0a0a0a] text-black dark:text-white transition-colors duration-500 selection:bg-black selection:text-white dark:selection:bg-white dark:selection:text-black">
            ...
            <section className="relative w-full h-[65vh] md:h-[75vh] flex items-center justify-center overflow-hidden bg-[#0a0a0a]">
                <Image
                    src="/assets/generated/shoes-hero.png"
                    alt="Shoes Collection Hero"
                    fill
                ...
                <div className="relative z-10 text-center px-4 w-full max-w-5xl mt-20">
                    ...
                    <motion.h1
                        ...
                        className="text-6xl md:text-8xl lg:text-[10rem] font-serif font-black text-white uppercase tracking-tighter leading-none mb-8"
                    >
                        Walk <br/> <span className="italic text-white/50 text-5xl md:text-7xl lg:text-8xl">The Talk.</span>
                    </motion.h1>
                </div>
            </section>
                ...
                <div className="flex flex-col lg:flex-row gap-12 lg:gap-24">
                    ...
                    <aside className="hidden lg:block w-64 shrink-0 sticky top-32 h-fit space-y-12">
                        ...
                        <div className="space-y-4">
                            <input
                                type="text" placeholder="Search footwear..."
                        ...
                        {brandsList.length > 0 && (
                            <div className="space-y-6">
                                <h3 className="text-[10px] font-bold uppercase tracking-widest text-black/50 dark:text-white/50">Brands</h3>
                                <div className="flex flex-wrap gap-2">
                                    {brandsList.map(b => (
                                        <button
                                            key={b} onClick={() => toggleBrand(b)}
                                            className={`px-4 py-2 rounded-full text-xs font-medium border transition-colors ${filterState.brands.includes(b) ? 'bg-black text-white dark:bg-white dark:text-black border-transparent' : 'border-black/10 dark:border-white/10 hover:border-black dark:hover:border-white'}`}
                                        >
                                            {b}
                                        </button>
                                    ))}
                    ...
                    <div className="flex-1">
                        <div className="flex flex-col sm:flex-row justify-between items-end mb-16 gap-6">
                            <h2 className="text-4xl font-serif font-black tracking-tighter">
                                The <br/> <span className="italic text-black/40 dark:text-white/40">Footwear</span>
                            </h2>
                        ...
                        {loading ? (
                            ...
                            <div className="py-32 text-center">
                                <p className="text-2xl font-serif italic text-black/50 dark:text-white/50 mb-6">No footwear match your criteria.</p>
                ...
                {mobileFiltersOpen && (
                    ...
                    >
                        <div className="flex justify-between items-center mb-12">
                            <h2 className="text-3xl font-serif font-black uppercase tracking-tighter">Filter <br/><span className="italic text-black/40 dark:text-white/40">Footwear</span></h2>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-12 pb-24">
                            ...
                            <div>
                                ...
                                <input type="range" min="0" max="2500" value={filterState.priceRange[1]} onChange={(e) => handleFilterChange('priceRange', [0, Number(e.target.value)])} className="w-full accent-black dark:accent-white" />
                            </div>
                        </div>
