...
export default function AccessoriesPage() {
    ...
    // 1. Fetch Accessories specifically on page load
    useEffect(() => {
    ...
    const materialsList = useMemo(() => {
        const m = new Set();
        products.forEach(p => p?.material && m.add(p.material));
        return Array.from(m);
    }, [products]);
    ...
    const resetFilters = () => setFilterState({ query: '', sort: 'featured', gender: 'All', priceRange: [0, 1000], materials: [] });
    ...
    return (
        <div className="min-h-screen bg-background text-foreground font-sans transition-colors duration-300">
            ...
            <section className="relative h-[50vh] md:h-[60vh] w-full overflow-hidden bg-secondary">
                <Image
                    src="https://images.unsplash.com/photo-1606760227091-3dd870d97f1d?q=80&w=1974&auto=format&fit=crop"
                ...
                <div className="relative z-10 h-full flex flex-col justify-center items-center text-center px-4 max-w-4xl mx-auto">
                    ...
                    >
                        From statement jewelry to essential leather goods, explore our curated selection of premium accessories designed to elevate every outfit.
                    </motion.p>
                </div>
            </section>
                ...
                <div className="flex flex-col lg:flex-row gap-12">
                    ...
                    <aside className="hidden lg:block w-72 flex-shrink-0 sticky top-24 h-fit space-y-10 pr-6 border-r border-border/50">
                        ...
                        {materialsList.length > 0 && (
                            <div className="space-y-4">
                                <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Material</h3>
                                <div className="flex flex-wrap gap-2">
                                    {materialsList.map(m => (
                                        <button
                                            key={m}
                                            onClick={() => toggleMaterial(m)}
                                            className={`px-3 py-1.5 rounded-md text-xs font-medium border transition-all ${filterState.materials.includes(m)
                    ...
                    <div className="flex-1">
                        ...
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-32 text-center bg-secondary/10 rounded-2xl border border-dashed border-border">
                                ...
                                <p className="text-muted-foreground mb-8 max-w-sm">We could not find any accessories matching your current filters.</p>
                                <button
